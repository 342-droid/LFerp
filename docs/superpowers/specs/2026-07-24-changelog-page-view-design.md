...
## 呈现层 1：changelog.html 提交为主体视图（07-25 定稿，取代最初的页面聚合）

- 每天一个 section，按提交列条目：类型徽章 + 标题 + 作者·时间；条目下拆分该提交影响的页面——
  每行 = module 徽章 + 页面名（直达链接）+ 该页人话说明 + 对比入口（看对比/无界面变化/新增）。
- 无说明且截图无差异的页面行弱化沉底；提交级说明（通用/未归属）以引用条样式显示在页面拆分前。
- 今天/昨天（东八区）默认展开页面拆分，更早的收起为「N 个页面 ▾」。
- 选型理由：一个提交动多页时页面聚合视图会重复展示同一标题；提交=一个完整业务动作，
  配按页面说明后以提交为叙事单元更自然（用户 07-25 拍板）。
