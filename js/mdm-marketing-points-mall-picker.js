 ...
 */
(function () {
  var DRAWER_ID = 'mdmPointsMallPickerDrawer';
  ...
  function closeDrawer() {
    var backdrop = document.querySelector('[data-points-mall-picker-backdrop]');
  ...
  function renderSidebar() {
    var listEl = document.getElementById('pointsMallPickerCatList');
    if (!listEl) return;
  ...
  function renderGrid() {
    var gridEl = document.getElementById('pointsMallPickerGrid');
    var totalEl = document.getElementById('pointsMallPickerTotal');
    var countEl = document.getElementById('pointsMallPickerSelectedCount');
    var confirmBtn = document.getElementById('pointsMallPickerConfirm');
    if (!gridEl) return;
  ...
  function buildDrawerHtml() {
    return (
      '<div class="store-drawer-backdrop proxy-library-drawer-backdrop" data-points-mall-picker-backdrop></div>' +
      ...
      '    <button type="button" class="store-drawer__close" data-points-mall-picker-close aria-label="关闭">&times;</button>' +
      '  </header>' +
      ...
      '      <ul class="proxy-library-cat-list" id="pointsMallPickerCatList"></ul>' +
      '    </aside>' +
      '    <div class="proxy-library-drawer__main">' +
      '      <div class="proxy-library-drawer__toolbar mkt-points-mall-picker-toolbar">' +
      ...
      '          <input type="text" class="proxy-library-drawer__search-input" id="pointsMallPickerSearch" placeholder="搜索商品名称、编码..." autocomplete="off">' +
      '        </div>' +
      '        <label class="mkt-points-mall-picker-added">' +
      '          <span>是否添加</span>' +
      '          <select id="pointsMallPickerAddedFilter" aria-label="是否添加">' +
      '            <option value="">全部</option>' +
      '            <option value="added">已添加</option>' +
      '            <option value="not_added">未添加</option>' +
      '          </select>' +
      '        </label>' +
      '        <span class="proxy-library-drawer__total" id="pointsMallPickerTotal">共 0 件商品</span>' +
      '      </div>' +
      '      <div class="proxy-library-drawer__grid-wrap">' +
      '        <div class="proxy-library-drawer__grid" id="pointsMallPickerGrid"></div>' +
      '      </div>' +
      '    </div>' +
      '  </div>' +
      ...
      '      <button type="button" class="erp-btn erp-btn--primary" id="pointsMallPickerConfirm" data-points-mall-picker-confirm disabled>确认添加</button>' +
      '    </div>' +
      '  </footer>' +
      '</aside>'
    );
  ...
  function bindDrawerEvents(onConfirm) {
    ...
    document.querySelectorAll('[data-points-mall-picker-close], [data-points-mall-picker-cancel]').forEach(function (btn) {
      btn.addEventListener('click', closeDrawer);
    });

    var searchEl = document.getElementById('pointsMallPickerSearch');
    if (searchEl) {
    ...
    var addedFilterEl = document.getElementById('pointsMallPickerAddedFilter');
    if (addedFilterEl) {
    ...
    var catList = document.getElementById('pointsMallPickerCatList');
    if (catList) {
