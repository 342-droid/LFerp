...
(function () {
  ...
  var segmentId = params.get('segmentId') || '';
  var segmentName = params.get('name') || '';
      ...
      if (segmentId) {
        ...
        if (raw) {
          ...
          for (var i = 0; i < list.length; i++) {
            if (list[i].id === segmentId && list[i].name) return list[i].name;
          }
        }
      }
    } catch (e) { /* ignore */ }
    if (segmentName && segmentName !== '未命名人群') return segmentName;
    return segmentId ? ('人群 ' + segmentId) : '人群列表';
  }
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = (resolveSegmentName() || '人群列表') + '_成员_' + Date.now() + '.csv';
  ...
  function bindEvents() {
    ...
    if (back) back.href = pageHref('mdm_member_segment.html');
  ...
  function init() {
    document.getElementById('segmentNameLabel').textContent = resolveSegmentName();
    if (segmentId) {
      var seed = segmentId.charCodeAt(segmentId.length - 1) || 0;
