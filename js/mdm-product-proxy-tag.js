(function () {
  var store = window.MdmProxyTagStore;
  var TAG_NAME_MAX = 5;
  ...
  function closeModal() {
    var backdrop = document.querySelector('[data-proxy-tag-modal]');
    if (backdrop) backdrop.remove();
  }

  function closeWarmConfirmModal() {
    var backdrop = document.querySelector('[data-proxy-tag-warm]');
    if (backdrop) backdrop.remove();
  }
  ...
  function openWarmConfirm(message, onConfirm) {
    ...
    backdrop.setAttribute('data-proxy-tag-warm', '1');
    backdrop.innerHTML =
  ...
  function openTagModal(options) {
    ...
    var backdrop = document.createElement('div');
    backdrop.className = 'erp-modal-backdrop product-proxy-tag-modal-backdrop';
    backdrop.setAttribute('data-proxy-tag-modal', '1');
    backdrop.innerHTML =
      '<div class="erp-modal product-proxy-tag-modal">' +
      '  <div class="erp-modal__header">' +
      ...
      '  </div>' +
      '  <div class="erp-modal__body">' +
      '    <div class="product-proxy-tag-form__row">' +
      '      <label class="product-proxy-tag-form__label" for="proxyTagName">' +
      '        <span class="product-add-field__req">*</span>标签名称' +
      '      </label>' +
      '      <div class="product-proxy-tag-form__control">' +
      '        <input class="product-proxy-tag-form__input" id="proxyTagName" type="text" placeholder="请输入标签名称" maxlength="' + TAG_NAME_MAX + '" value="' + escapeHtml(initialName) + '">' +
      '        <span class="product-proxy-tag-form__counter" id="proxyTagNameCounter">' + initialName.length + ' / ' + TAG_NAME_MAX + '</span>' +
      '      </div>' +
      '    </div>' +
      '  </div>' +
      '  <div class="erp-modal__footer">' +
    ...
    var modal = backdrop.querySelector('.product-proxy-tag-modal');
    backdrop.addEventListener('click', function (ev) {
    ...
    var nameInput = backdrop.querySelector('#proxyTagName');
    var counterEl = backdrop.querySelector('#proxyTagNameCounter');
  ...
  function renderTable() {
    var tbody = document.getElementById('proxyTagTableBody');
    var emptyEl = document.getElementById('proxyTagEmpty');
    if (!tbody) return;
    ...
    if (!pageItems.length) {
      ...
      tbody.innerHTML = pageItems.map(function (tag) {
        return (
          ...
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--sort">' + tag.sort + '</td>' +
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--id">' + escapeHtml(tag.id) + '</td>' +
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--name">' + escapeHtml(tag.name) + '</td>' +
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--count">' + tag.bind_count + '</td>' +
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--time">' + escapeHtml(tag.created_at) + '</td>' +
          '<td class="product-proxy-tag-table__td product-proxy-tag-table__td--action">' + renderActions(tag) + '</td>' +
          '</tr>'
        );
      }).join('');
    }
  ...
  function renderPagination() {
    var totalEl = document.getElementById('proxyTagPaginationTotal');
    var pagesEl = document.getElementById('proxyTagPaginationPages');
    var gotoEl = document.getElementById('proxyTagPageGoto');
  ...
  function bindEvents() {
    var addBtn = document.getElementById('proxyTagAddBtn');
    if (addBtn) {
    ...
    var pageSizeEl = document.getElementById('proxyTagPageSize');
    if (pageSizeEl) {
    ...
    var pagination = document.getElementById('proxyTagPagination');
    if (pagination) {
