...
(function () {
  ...
  function chipStyle(color) {
    var normalized = safeColor(color);
    if (!normalized) return '';
    return ' style="--bt-color:' + normalized + '"';
  }
  ...
  function closeLayer(layer) {
    if (!layer) return;
    var restore = layer._btRestoreFocus;
    if (layer.parentNode) layer.parentNode.removeChild(layer);
    if (restore && restore.isConnected && typeof restore.focus === 'function') restore.focus();
  }

  function activateLayer(layer, panel) {
    var previous = document.activeElement;
    layer._btRestoreFocus = previous;
    layer.addEventListener('keydown', function (event) {
      ...
      var focusable = Array.prototype.filter.call(
        panel.querySelectorAll('button, input, select, textarea, [tabindex]:not([tabindex="-1"])'),
        function (item) { return !item.disabled && !item.hidden && item.offsetParent !== null; }
      );
    ...
    var initial = panel.querySelector('button, input, select, textarea');
  ...
  function openManager(type, title) {
    ...
    layer.innerHTML =
      ...
      '<aside class="bt-manager" role="dialog" aria-modal="true" data-business-tag-manager aria-label="' + escapeHtml(title) + '">' +
    ...
    activateLayer(layer, layer.querySelector('[data-business-tag-manager]'));
    ...
    function renderTable() {
      ...
      body.innerHTML = tags.map(function (item) {
        return '<tr><td><span class="bt-chip' + (item.color ? ' has-color' : '') + '"' + chipStyle(item.color) +
          ...
          '<td>' + (safeColor(item.color) ? '<span class="bt-color-value"><i style="background:' + safeColor(item.color) + '"></i>' + escapeHtml(safeColor(item.color)) + '</span>' : '默认') + '</td>' +
  ...
  function openBinding(type, resourceIds, title) {
    ...
    layer.innerHTML =
      ...
      '</section>';
    document.body.appendChild(layer);
    activateLayer(layer, layer.querySelector('.bt-binding'));
