/**
 * 营销活动 — 秒杀商品编辑（复刻直播排品编辑，直播库存改为活动库存）
 */
(function () {
  'use strict';
  ...
  function listHref() {
    ...
    var working = Store.getWorking() || {};
    var qs = ['tab=products', 'mode=' + encodeURIComponent(mode)];
    if (working.id) qs.unshift('id=' + encodeURIComponent(working.id));
  ...
  function findProduct(pid) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === pid) return { item: list[i], index: i, list: list, working: working };
    }
    return null;
  }
  ...
  function normalizePoint(mode) {
    ...
    if (mode === 'cash' || mode === 'CASH') return 'cash';
    return '';
  }
  ...
  function normalizeSku(raw, productImg) {
    ...
    return {
      ...
      salePrice: price == null ? '' : price,
      linePrice: raw.linePrice != null ? raw.linePrice : raw.marketPrice,
      minQty: raw.minQty != null && raw.minQty !== '' ? raw.minQty : '',
      sellableStock: raw.sellableStock,
      spotStock: raw.spotStock,
      ...
      liveStock: raw.activityStock != null ? raw.activityStock : raw.liveStock != null ? raw.liveStock : raw.stock,
  ...
  function ensureSkus(p) {
    ...
    return list.map(function (s) {
      var sku = normalizeSku(s, p.img);
        ...
        var sum = window.MdmSkuWhStock.attachToSku(sku, { sessionId: 'seckill' });
        var cap = (sum.remainTotal || 0) + (sum.sessionReservedTotal || 0);
        if (sku.sellableStock == null || sku.sellableStock === '') sku.sellableStock = sum.sellableTotal;
        var hasStock = sku.liveStock !== '' && sku.liveStock != null;
        if (hasStock && Number(sku.liveStock) > cap) {
          sku.liveStock = cap;
        }
      }
      return sku;
  ...
  function renderSkuCard(sku) {
    ...
    return (
      ...
      (normalizePoint(sku.pointExchange) === 'points' ? '' : moneyHtml('秒杀价', 'salePrice', sku.salePrice)) +
      moneyHtml('划线价', 'linePrice', sku.linePrice) +
      fieldHtml('起售量', 'minQty', sku.minQty) +
      fieldHtml('现货库存', 'spotStock', sku.spotStock, true) +
      fieldHtml('实际库存', 'sellableStock', sku.sellableStock, true) +
      fieldHtml('活动库存', 'liveStock', sku.liveStock, false, 'is-live-stock') +
      '<p class="product-proxy-spec__stock-tip product-proxy-spec__stock-tip--span">划线价选填。真实库存有上限时，可下单库存 = min(实际库存, 活动库存)' +
      (Store.orderableStockOf
        ? '，当前可下单 ' + Store.orderableStockOf(sku)
        : '') +
      '。</p>' +
      '</div></div>' +
  ...
  function loadForm() {
    ...
    var titleText = mode === 'view' ? '查看秒杀商品' : '编辑秒杀商品';
    var tabBtn = document.getElementById('skProdEditTab');
    var titleEl = document.getElementById('skProdEditTitle');
    if (tabBtn) tabBtn.textContent = titleText;
    if (titleEl) titleEl.textContent = titleText;
    document.title = '冷丰WMS - ' + titleText;

    var found = productId ? findProduct(productId) : null;
    ...
    var dm = product.deliveryMode === 'pickup' ? 'pickup' : product.deliveryMode === 'express' ? 'express' : '';
    document.querySelectorAll('input[name="deliveryMode"]').forEach(function (el) {
      el.checked = el.value === dm;
    });

    var salesMode = product.displaySalesMode === 'CUSTOM' || product.displaySalesMode === 'ACTUAL' ? product.displaySalesMode : '';
    document.querySelectorAll('input[name="displaySalesMode"]').forEach(function (el) {
      el.checked = el.value === salesMode;
    });
    ...
    renderSkuCards();
    if (mode === 'view') {
      document.querySelectorAll('#schedEditForm input, #schedEditForm select, #schedEditForm textarea, #schedEditForm [contenteditable]').forEach(function (el) {
        el.disabled = true;
      ...
      var resetBtn = document.getElementById('pResetSkuBtn');
  ...
  function validate() {
    ...
    if (!(document.querySelector('input[name="displaySalesMode"]:checked') || {}).value) {
      return toast('请选择展示销量', 'warning'), false;
    }
    var salesMode = (document.querySelector('input[name="displaySalesMode"]:checked') || {}).value;
    if (salesMode === 'CUSTOM' && !((document.getElementById('pDisplaySales') || {}).value || '').trim()) {
      return toast('请填写自定义展示销量', 'warning'), false;
    }
    readSkuCards();
    ...
    for (var i = 0; i < selectedSkuIds.length; i++) {
      ...
      if (!s) continue;
      var modePt = normalizePoint(s.pointExchange);
      if (!s.saleUnit) return toast('请选择 ' + (s.displayName || s.id) + ' 的售卖单位', 'warning'), false;
      if (!s.limitConfig) return toast('请选择 ' + (s.displayName || s.id) + ' 的限购配置', 'warning'), false;
      if (!s.pointExchange) return toast('请选择 ' + (s.displayName || s.id) + ' 的积分兑换', 'warning'), false;
      if (modePt !== 'points' && (s.salePrice === '' || s.salePrice == null || isNaN(Number(s.salePrice)))) {
        return toast('请补充 ' + (s.displayName || s.id) + ' 的秒杀价', 'warning'), false;
      }
      if (s.minQty === '' || s.minQty == null || isNaN(Number(s.minQty))) {
        return toast('请补充 ' + (s.displayName || s.id) + ' 的起售量', 'warning'), false;
      }
      if (s.liveStock === '' || s.liveStock == null || isNaN(Number(s.liveStock))) {
        return toast('请补充 ' + (s.displayName || s.id) + ' 的活动库存', 'warning'), false;
      }
      if (window.MdmSkuWhStock && window.MdmSkuWhStock.summarize) {
        var stockSum = window.MdmSkuWhStock.summarize(s, { sessionId: 'seckill' });
        var cap = (stockSum.remainTotal || 0) + (stockSum.sessionReservedTotal || 0);
        if (cap > 0 && Number(s.liveStock) > cap) {
          return toast(
            (s.displayName || s.id) + ' 活动库存不能超过配送仓剩余可售（' + cap + '）',
            'warning'
          ), false;
        }
      }
  ...
  function bindEvents() {
    ...
    document.getElementById('pSaveBtn').addEventListener('click', function () {
      if (!validate()) return;
      applyToProduct();
      Store.upsertWorkingProduct(product);
      toast('保存成功');
      window.location.href = listHref();
    });
