...
(function () {
  ...
  function mountCapabilityPage() {
    ...
    var modules = [
      ...
      {
        code: 'MDM_PRODUCT_TAG',
        title: '商品主数据',
        desc: '商品域维护统一商品标签库；截单等业务在各自配置中引用标签。',
        name: '商品标签',
        note: '现有业务体系 · 标签规则由对应业务配置引用',
        icon: '商'
      },
      {
        code: 'AFTER_SALE_TAG',
  ...
  function addHeaderAfterText(row, anchorText, key, text) {
    ...
    if (key === 'after-sale') th.setAttribute('data-preference-key', 'afterSaleTags');
    if (key === 'member') th.setAttribute('data-preference-key', 'memberTags');
