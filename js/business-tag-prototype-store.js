 ...
 * 原型约束：
 * 1. 系统管理员只负责按业务模块开通能力；
 * 2. 门店、商品、售后分别维护自己的标签库；会员标签复用会员域现有能力；
 * 3. 颜色是可选展示属性，不参与标签身份；
...
(function () {
  ...
  function normalize(data) {
    ...
    if (!Object.prototype.hasOwnProperty.call(modules, 'MDM_PRODUCT_TAG') &&
        Object.prototype.hasOwnProperty.call(legacy, 'MDM_PRODUCT_MANAGEMENT:SELF')) {
      modules.MDM_PRODUCT_TAG = legacy['MDM_PRODUCT_MANAGEMENT:SELF'];
    }
    ...
    ['STORE', 'AFTER_SALE'].forEach(function (type) {
      ...
      next.catalogs[type].forEach(function (item) {
  ...
  function addTag(type, values) {
    ...
    var rawColor = String((values && values.color) || '').trim();
    if (!name) throw new Error('请输入标签名称');
    if (rawColor && !normalizeColor(rawColor)) throw new Error('标签颜色仅支持 6 位十六进制色值');
  ...
  function ensureDemoBinding(type, resourceId, rowIndex) {
    ...
    if (!id) return [];
    var data = read();
    var bindings = data.bindings[type] || {};
    if (Object.prototype.hasOwnProperty.call(bindings, id)) return (bindings[id] || []).slice();
