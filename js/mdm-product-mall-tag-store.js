 ...
 */
(function () {
  var STORAGE_KEY = 'mdm_mall_tag_list_v1';
  var PRODUCT_LIST_KEY = 'mdm_mall_product_list_v1';
  ...
  function readMallProducts() {
    try {
      var raw = sessionStorage.getItem(PRODUCT_LIST_KEY);
      if (!raw) return [];
      var parsed = JSON.parse(raw);
  ...
  function updateTag(id, payload) {
    ...
    if (oldName !== name) {
      ...
      if (changed) {
        try {
          sessionStorage.setItem(PRODUCT_LIST_KEY, JSON.stringify(products));
        } catch (e) { /* ignore */ }
      }
    }
